...
%     global model_C_data_2mm
    ...
    threshs = optimizableVariable('threshold', [1e-7 1e-1],'Transform','log');
    ...
    grid_IOUs(1, grid_i) = results.MinObjective;
    grid_IOUs(2, grid_i) = results.XAtMinObjective{1, 1};
end

%%
function obj = get_best_threshold(threshold)
    global lesion_metadata;
%     IOUs = [];
    c_maxdepths = [];
    maxdepths = [];
    c_avgwidths = [];
    ...
    for les_i = 1:22 % go to 30 if want to include 0s
        ...
        cntr = get_model_contour(lmd.aperture, lmd.intensity, threshold.(1));
