%% Histology Image Analysis for PT Lesions - 3D Lesion Reconstruction

% Authors: Karam Khateeb and Mona Rahimi
% The purpose of this script is to reconstruct the lesions in 3D space
% based on the Nissl staining analysis


%% Manual Inputs for Each Animal
...
% This is a generic script generated for the scripts of past PT scripts to
% generalize to future PTs

%% Manual Inputs for Each Animal

% Slide nomenclature: box#.well#.pass# (posterior -> anterior)
% It's easier to conver the above nomenclature into a single 1:numSlides
% array

% UPDATE THESE FOR EACH ANIMAL
% Slide nomenclature: box#.well#.pass# (anterior -> posterior)
% boxes = 1:4;
% wells = [1 10 19 28];
% passes = 1:2;

PTNum = 6; % which PT is this?
...
% Input the resolution of the images
DPI = 3200;

% Set the scale in the ML and DV axis
% DPI * ResizeFactor = 3200 * 0.1 = 320 (pixels per inch)
% Convert pixel size to mm --> 0.079 mm
% May need to use PT_Reconstruct_Scaling.m to get accurate delta
delta = (25.4/(DPI * ResizeFactor)); % in mm

% Input the spacing between slides
dz = 0.45; % in mm

% set filepath
filepath = [filesep 'Users' filesep 'Mona Rahimi' filesep 'Documents' filesep 'Yazdan Lab' filesep 'PT' num2str(PTNum)];

% set initial fixed image to start image registration
fix1 = [filepath filesep filesep 'Slides' filesep '1.tif']; % just the file name of the first slide

% rotation needed for first fixed image (to get it straight)?
degreeRot = -5; % Determined manually

% set the ROI for all scanned slides
cropRect = [2515 30 7096 6330]; % ROI determined manually [XYWH]

% choose a small patch of background white color
whitePatch = [19 39 27 25];

% choose the rectangle for the top half of the registered slides to reduce
% file sizes
topRec = [1 1 760 421];

% Every how many slides should we change the fixed image?
fixInt = 4; % play around with this to reduce spiraling
...
% rotate fixed image so that it's straight
...
for islide = slides
    ...
    currentFile = ['Slides' filesep num2str(islide) '.tif'];
    if exist(currentFile, 'file') == 0
        continue;
    end 
    currentIm = imread(['Slides' filesep num2str(islide) '.tif']);
    ...
    % sample code for control point registration if needed for subset of slides
...
%     end 
    ...
    % Grabs white patch
    patch = imcrop(croppedIm, whitePatch);
    patchRed = patch(:,:,1);
    patchGreen = patch(:,:,2);
    patchBlue = patch(:,:,3);
    ...
    topCrop = imcrop(registeredIm, topRec);
    figure; imshow(topCrop); title('topCrop image');

    % resave the image in PT5_RegSlides folder
    imwrite(topCrop, [filepath filesep 'PT' num2str(PTNum) '_RegSlides' filesep num2str(islide) '.tiff']);

    disp(['Saved registered ' num2str(islide) '.tif']);

    % Switch the fixed image to registered image every fixInt image
    register = register +1;
    if mod(register,fixInt) == 0
...
% After saving registered images, use Adobe Photoshop magnetic lasso tool
% to outline lesions. Then save two images for each slide: 1 with the
% lesion filled in with background color, and another the color of gray
% matter
...
fullBrain = {}; % cell to store full brain outline and lesion-less outline

tic
for iSlide = slides

    % Load registered image - CHANGE ADDRESSES FOR PT2
    currentIm = imread(['PT' num2str(PTNum) '_RegSlides' filesep num2str(ibox) '.' num2str(iwell) '.' num2str(ipass) '.tif']); 
    noLesion = imread(['PT' num2str(PTNum) '_FinalInclude' filesep num2str(ibox) '.' num2str(iwell) '.' num2str(ipass) '.tif']); %% PT4_FinalInclude
    yesLesion = imread(['PT' num2str(PTNum) '_FinalExclude' filesep num2str(ibox) '.' num2str(iwell) '.' num2str(ipass) '.tif']); %% PT4_FinalExclude

    disp(['Reading ' num2str(ibox) '.' num2str(iwell) '.' num2str(ipass) '.tif']);

    % combination of filtering methods as needed to get best edge-detection
    % (un-comment the methods you would use)
...
%     patch = imcrop(currentIm, [120 100 220 200]); % divide [XYWH]/0.075 for full size CHANGE FOR PT2
...
%         else
    ...
    saveas(gcf, [filepath filesep 'PT' num2str(PTNum) '_BrainBounds' filesep num2str(ibox) '.' num2str(iwell) '.' num2str(ipass) '.tif']);
    iSlide = iSlide +1;
    fullBrain{iSlide,1} = cleanBoundariesNL{1,1};
    fullBrain{iSlide,2} = cleanBoundariesYL{1,1};
    toc
    close gcf

end

save(['PT' num2str(PTNum) '_LesionBounds.mat'], 'fullBrain', 'currentIm');
...
saveas(gcf, ['PT' num2str(PTNum) '_ScaledLesionsFigure']);
...
title('Scaled Brain');
xlabel('Medial-lateral Axis (mm)'); ylabel('Dorsal-Ventral Axis (mm)'); zlabel('Anterior-Posterior Axis (mm)');
saveas(gcf, ['PT' num2str(PTNum) '_ScaledBrainFigure']);
...
xlabel('Medial-lateral Axis (mm)'); ylabel('Dorsal-Ventral Axis (mm)'); zlabel('Anterior-Posterior Axis (mm)');
saveas(gcf, ['PT' num2str(PTNum) '_ScaledInterpLesionsFigure']);
...
xlabel('Medial-lateral Axis (mm)'); ylabel('Dorsal-Ventral Axis (mm)'); zlabel('Anterior-Posterior Axis (mm)');
saveas(gcf, ['PT' num2str(PTNum) '_ScaledInterpBrainFigure']);
...
saveas(gcf, ['PT' num2str(PTNum) '_3DReconstructionOverlayTan']);
set(gcf,'color','w');
set(gca, 'fontsize', 18);
...
CaptureFigVid(viewZ, ['PT' num2str(PTNum) 'ReconstructTan'],OptionZ)
