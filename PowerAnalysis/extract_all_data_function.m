%Devon took the example script, modified it, and turned it into a function 
%for retrieving and viewing data.
%
%example command line use:
% close all;clearvars;[x,y]=extract_all_data_function('Raw',4,true,'C:\Users\user\Trellis\dataFiles\20191113\datafile0001.ns5');
% close all;clearvars;[x,y]=extract_all_data_function('LFP',6,true,'C:\Users\user\Trellis\dataFiles\20191113\datafile0001.ns2');

%Note that each channel group on the physical Nomad (A,B,C,D) is apparently
%allotted 32*4 channels even if fewer than all 4 plugs are in use. So,
%channels for group B start at 32*4+1 regardless of group A's
%configuration.

%TODO Modify the script to return data from all channels.
%TODO Modify the script so that smaller sections of the data can be
%extracted.
%TODO Test all of the different parameter combinations.
%TODO Ensure that the different data types (Raw, LPF, etc.) pull from the
%correct file types (.ns5, .ns2, etc.)

function [x,y]=extract_all_data_function(dataType,dataChannelArray,plotStatus,completeFilePath)
x=[];y=[];
...
for dataChannel=dataChannelArray
...
switch dataType    
    ...
    case {'Spike' 'LFP' 'Hi-Res' 'Raw' 'Analog 30k' 'Analog 1k'}
        ...
        if isempty(EntityIndices),disp(['There is no channel number ',num2str(dataChannel)]),continue,end
    ...
    otherwise
        disp(['There is no data type ',dataType])
        return
end
...
switch dataType            
    ...
    case {'LFP' 'Hi-Res' 'Raw' 'Analog 30k' 'Analog 1k'}
        ...
        x=analogInputDataTime_s;
        y=analogInputData;
        if strcmp(dataType, 'Analog')
            analogInputData_mV = analogInputData; clear analogInputData;
            if plotStatus; figure;plot(analogInputDataTime_s,analogInputData_mV); xlabel('Time (s)'); ylabel ([dataType ' (mV)']); end;
        else            
            analogInputData_uV = analogInputData; clear analogInputData;
            if plotStatus; figure;plot(analogInputDataTime_s,analogInputData_uV); xlabel('Time (s)'); ylabel ([dataType ' (uV)']); end;
        end
